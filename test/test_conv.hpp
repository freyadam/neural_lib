...
TEST(ConvTest, InputOutput) {
    ...
    nl::block_ptr b = std::make_shared<nl::Block>("b", 2, 2, 2);
...
TEST(ConvTest, Forward1) {
    ...
    nl::block_ptr b1 = std::make_shared<nl::Block>("b1", 1, 2, 2);
    nl::Conv c1("c1", "relu", b1, 1, 2);

    nl::block_ptr out1 = c1.outputs()["c1_out"];
    nl::block_ptr w1 = c1.inputs()["c1_w0"];
    nl::block_ptr thr1 = c1.inputs()["c1_thr0"];    
...
TEST(ConvTest, Forward2) {
    ...
    nl::block_ptr b = std::make_shared<nl::Block>("b", 1, 3, 3);
    nl::Conv c("c", "linear", b, 2, 1, 0, 2);

    nl::block_ptr out = c.outputs()["c_out"];
    nl::block_ptr w1 = c.inputs()["c_w0"];
    nl::block_ptr thr1 = c.inputs()["c_thr0"];    
    nl::block_ptr w2 = c.inputs()["c_w1"];
    nl::block_ptr thr2 = c.inputs()["c_thr1"];    
...
TEST(ConvTest, Forward3) {
    ...
    nl::block_ptr out = c.outputs()["c_out"];
    nl::block_ptr w = c.inputs()["c_w0"];
    nl::block_ptr thr = c.inputs()["c_thr0"];    
